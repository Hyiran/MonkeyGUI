...
public class GUI_RandomKey_RandomTime extends JFrame implements Runnable {

	private JPanel userJPanel, userJPanel1, userJPanel2;
	private JButton RandomKey_RandomTime_OkJButton, btnSelect,
			RandomKey_RandomTime_BackJButton;
	...
	private JLabel IP2, LoopCount2, LoopTime2,LoopTime3,
			pictureJlabelBackground, pictureJLabel, LogoJLabel;// 文本框后说明
	private JTextField IP, Port, SeleFolder, LoopTime,LoopTime4, LoopCount;
	...
	public GUI_RandomKey_RandomTime() {
	...
	private void createUserInterface() {
		...
		SeleFolder1.setBounds(boundsuserJPanel.width / 50,
				boundsuserJPanel.height / 10, boundsuserJPanel.width / 2,
				(boundsuserJPanel.height / 100) * 45);
		...
		LoopTime.setBounds(((boundsuserJPanel1.width / 24) * 8),
				LoopTimebounds.y, boundsuserJPanel1.width / 15,
				(boundsuserJPanel.height / 100) * 45);
		LoopTime.setFont(new Font("楷体", Font.BOLD,
				(boundsuserJPanel.height / 100) * 22));
		LoopTime.setForeground(new Color(0x43,0x6E,0xEE));
		LoopTime4 = new JTextField();
		LoopTime4.setBounds(((boundsuserJPanel1.width / 24) * 10),
				LoopTimebounds.y, boundsuserJPanel1.width / 15,
				(boundsuserJPanel.height / 100) * 45);
		LoopTime4.setFont(new Font("楷体", Font.BOLD,
				(boundsuserJPanel.height / 100) * 22));
		LoopTime4.setForeground(new Color(0x43,0x6E,0xEE));
		userJPanel1.add(LoopTime);
		userJPanel1.add(LoopTime4);
		LoopTime1 = new JLabel("键值间隔:");
		LoopTime1.setBounds(boundsuserJPanel1.width / 50,
		...
		LoopTime2 = new JLabel("单位秒,值>0");
		LoopTime2.setBounds(((boundsuserJPanel1.width / 24) * 12),
				LoopTimebounds.y, boundsuserJPanel1.width,
				(boundsuserJPanel.height / 100) * 45);
		LoopTime2.setFont(new Font("楷体", Font.BOLD| Font.ITALIC,
				(boundsuserJPanel.height / 100) * 30));
		LoopTime2.setForeground(new Color(0x57,0x57,0x57));
		LoopTime3 = new JLabel("—");
		LoopTime3.setBounds(((boundsuserJPanel1.width / 24) * 9),
				LoopTimebounds.y, boundsuserJPanel1.width / 5,
				(boundsuserJPanel.height / 100) * 45);
		LoopTime3.setFont(new Font("楷体", Font.BOLD| Font.ITALIC,
				(boundsuserJPanel.height / 100) * 30));
		LoopTime3.setForeground(new Color(0x57,0x57,0x57));
		userJPanel1.add(LoopTime1);
		userJPanel1.add(LoopTime2);
		userJPanel1.add(LoopTime3);

		RandomKey_RandomTime_OkJButton = new JButton("确认");
		RandomKey_RandomTime_OkJButton.setBounds(bounds.x + bounds.width / 5,
				bounds.y + (bounds.height / 6) * 5, bounds.width / 9,
				bounds.height / 16);
		RandomKey_RandomTime_OkJButton.setContentAreaFilled(false);
		// 此颜色值随便设置，只起占位作用，但是必须加上此句，否则不会出现透明背景效果。
		RandomKey_RandomTime_OkJButton.setFont(new Font("楷体", Font.BOLD,
				((bounds.height / 6) / 100) * 40));
		RandomKey_RandomTime_OkJButton.setForeground(new Color(0x0A, 0x0A,
				0x0A));
		RandomKey_RandomTime_OkJButton.setFocusPainted(false);
		contentPane.add(RandomKey_RandomTime_OkJButton);
		RandomKey_RandomTime_OkJButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				int mapcount = 1;
				for (int i = 1; i <= n; i++) {
					try {
				...
				okJButtonAction.RandomKey_RandomTime_OkJButtonActionPerformed(
						event, SeleFolder.getText(), IP_Port,
						LoopCount.getText(), 0, 0, map,LoopTime.getText(),
						LoopTime4.getText(), "1", obj);
			}
		});

		RandomKey_RandomTime_BackJButton = new JButton("返回");
		RandomKey_RandomTime_BackJButton.setBounds(bounds.x
				+ (bounds.width / 5) * 3, bounds.y + (bounds.height / 6) * 5,
				bounds.width / 9, bounds.height / 16);
		RandomKey_RandomTime_BackJButton.setContentAreaFilled(false);
		; // 此颜色值随便设置，只起占位作用，但是必须加上此句，否则不会出现透明背景效果。
			// okJButton.setIcon(new
			// ImageIcon("bin/filechooser/image/rec.gif"));
		RandomKey_RandomTime_BackJButton.setFont(new Font("楷体", Font.BOLD,
				((bounds.height / 6) / 100) * 40));
		RandomKey_RandomTime_BackJButton.setForeground(new Color(0x0A, 0x0A,
				0x0A));
		RandomKey_RandomTime_BackJButton.setFocusPainted(false);
		contentPane.add(RandomKey_RandomTime_BackJButton);
		RandomKey_RandomTime_BackJButton
				.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent event) {
						GUI_Button_Monitor backJButtonAction = new GUI_Button_Monitor();
						backJButtonAction
								.RandomKey_RandomTime_BackJButtonActionPerformed(
										event, obj);
					}
				});
		...
		LogoJLabel.setText("随机键值、随机时间间隔");
		...
		Image imgbackground = Toolkit.getDefaultToolkit().getImage(
				GUI_RandomKey_RandomTime.class.getClassLoader().getResource(
	...
	public GUI_RandomKey_RandomTime obj = null;

	public void GetObject(GUI_RandomKey_RandomTime ob) {
	...
	public static void main(String[] args) {
		...
		GUI_RandomKey_RandomTime mylogin = new GUI_RandomKey_RandomTime();
